...
| 方法名称                         | 类型       | 参数说明                                             | 说明                                               |
|------------------------------|----------|--------------------------------------------------|--------------------------------------------------|
| KsLive.Tool.init             | `直播弹幕采集协议` | `liveUrl`：电脑网页版直播地址  `cookie`：快手直播网页端cookie      | `⚠️调用后续方法前必须先调用该方法进行初始化`                         |
| KsLive.Tool.wssServerStart   | `直播弹幕采集协议` | `liveUrl`：电脑网页版直播地址                              | 启动websocket客户端                                   |
| KsLive.Tool.getLiveRoomId    | `直播弹幕采集协议` |                                                  | 获取直播房间号id                                        |
| KsLive.Tool.getAnchorInfo    | `直播弹幕采集协议` |                                                  | 获取主播信息（如：主播个人信息、直播地址等）                           |
...
| KsLive.Tool.getUserCardInfoById    | `直播弹幕采集协议` | `principalId`：用户ID                                 | 根据用户ID查看用户基本信息                                   |
| KsLive.Tool.sendMsg          | `直播弹幕采集协议` | `content`发送内容 `liveStreamId`：房间号ID  `color`：内容颜色 | 直播发言💬                                           |
| KsLive.Tool.follow           | `直播弹幕采集协议` | `principalId`：用户ID `type`： 1关注 2取消关注             | 关注用户                                             |
